...
1.  **Динамический импорт:** Использование `importlib.import_module(f"{action}.main")` позволяет загружать разные скрипты в зависимости от URL (`/run/swap1/...` загружает `swap1/main.py`, `/run/swap2/...` загружает `swap2/main.py`).
2.  **Пар
